...
def invite_member(household_id):
    ...
    try:
        ...
        if existing_invite and not existing_invite.is_expired():
            return (
                jsonify({"error": "An active invite already exists for this email"}),
                400,
            )
...
def get_invite_details(token):
    ...
    try:
        ...
        if invite.status != "pending":
            return (
                jsonify({"error": f"This invite has already been {invite.status}"}),
                400,
            )
...
def accept_invite(token):
    ...
    try:
        ...
        if user.email != invite.invitee_email:
            return (
                jsonify({"error": "This invite was sent to a different email address"}),
                403,
            )
    ...
    try:
        ...
        if user_id == current_user_id:
            return (
                jsonify(
                    {
                        "error": "Owner cannot remove themselves. Transfer ownership or delete household instead."
                    }
                ),
                400,
            )
    ...
    try:
        ...
        if household.is_owner(user):
            return (
                jsonify(
                    {
                        "error": "Owner cannot leave household. Transfer ownership or delete household instead."
                    }
                ),
                400,
            )
