...
# Factory function to create models/tables with a given db instance
def create_household_models(db):
    # Association table for many-to-many relationship between users and households
    user_household = db.Table(
        "user_household",
        db.Column("user_id", db.Integer, db.ForeignKey("users.id"), primary_key=True),
        db.Column(
            "household_id", db.Integer, db.ForeignKey("households.id"), primary_key=True
        ),
    ...
    class Household(db.Model):
            ...
            if include_members:
                ...
                for member in self.members:
                    role_query = db.session.execute(
                        db.select(
                            user_household.c.role, user_household.c.joined_at
                        ).where(
                            db.and_(
                    ...
                    result["members"].append(
                        {
                            ...
                            "joined_at": (
                                role_query[1].isoformat()
                                if role_query and role_query[1]
                                else None
                            ),
                        }
                    )
        ...
        def is_owner(self, user):
            role_query = db.session.execute(
                db.select(user_household.c.role).where(
                    db.and_(
                        user_household.c.user_id == user.id,
                        user_household.c.household_id == self.id,
                    )
    ...
    class HouseholdInvite(db.Model):
        ...
        household = db.relationship(
            "Household", backref=db.backref("invites", lazy=True)
        )
    ...
    return Household, HouseholdInvite, user_household
