...
def list_transactions():
    ...
    transactions = (
        Transaction.query.filter_by(household_id=household_id)
...
def create_transaction():
    ...
    if transaction_type == "transfer":
        if not (to_account_id or to_fund_id):
            return (
                jsonify({"error": "Transfer requires to_account_id or to_fund_id"}),
                400,
            )

        if to_account_id:
            to_account = Account.query.filter_by(
                id=to_account_id, household_id=household_id
            ).first()
            if not to_account:
                return jsonify({"error": "Destination account not found"}), 404

        if to_fund_id:
            to_fund = Fund.query.filter_by(
                id=to_fund_id, household_id=household_id
            ).first()
            if not to_fund:
...
def auto_generate_transactions():
    ...
    try:
        ...
        db.session.rollback()
        return (
            jsonify({"error": f"Failed to create autopay transactions: {str(e)}"}),
            500,
        )
...
def get_transaction(transaction_id):
    ...
    transaction = Transaction.query.filter_by(
        id=transaction_id, household_id=household_id
    ).first()
    ...
    try:
        ...
        if original_fund_id:
            fund = Fund.query.filter_by(
                id=original_fund_id, household_id=household_id
            ).first()
            if fund:
        ...
        if "fund_id" in data:
            ...
            if fund_id:
                fund = Fund.query.filter_by(
                    id=fund_id, household_id=household_id
                ).first()
                if not fund:
                    return jsonify({"error": "Fund not found or access denied"}), 404
        ...
        if "bill_id" in data:
            ...
            if bill_id:
                bill = Bill.query.filter_by(
                    id=bill_id, household_id=household_id
                ).first()
                if not bill:
                    return jsonify({"error": "Bill not found or access denied"}), 404
        ...
        if transaction.fund_id:
            fund = Fund.query.filter_by(
                id=transaction.fund_id, household_id=household_id
            ).first()
            if fund:
                if transaction.transaction_type == "income":
                    ...
                    if fund.balance < abs(transaction.amount):
                        return (
                            jsonify(
                                {"error": "Insufficient fund balance for this expense"}
                            ),
                            400,
                        )
                    fund.balance -= abs(transaction.amount)
...
def process_recurring_transactions():
    ...
    try:
        for parent_tx in due_transactions:
            ...
            parent_tx.next_occurrence = parent_tx.calculate_next_occurrence(
                parent_tx.next_occurrence
            )
