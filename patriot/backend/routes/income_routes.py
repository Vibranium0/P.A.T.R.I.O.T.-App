...
def create_income_entry():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error creating income entry: {str(e)}"}
            ),
            500,
        )
...
def delete_income_entry(income_id):
    ...
    try:
        ...
        income_entry = Income.query.filter_by(
            id=income_id, household_id=household_id
        ).first()
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error deleting income entry: {str(e)}"}
            ),
            500,
        )
...
def get_income_summary():
    ...
    try:
        ...
        for source in source_summary:
            ...
            source_summary[source]["percentage"] = (
                round((total / total_income * 100), 2) if total_income > 0 else 0
            )

        # Sort by total amount (descending)
        sorted_sources = dict(
            sorted(
                source_summary.items(), key=lambda x: x[1]["total_amount"], reverse=True
            )
        )
