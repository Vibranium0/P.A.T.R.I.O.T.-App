...
from models import Fund, Transaction, Account
...
p.route("/", methods=["POST"])
_required()
def create_fund():
    """Create a new fund for the current household"""
ehold_id = get_current_household_id()
...
try:
    ...
    try:
            ...
            return jsonify({"error": "Invalid recurring amount"}), 40""
...
eck if fund name already exists for this household
    existing_fun.query.,filter_by(household_id=household_id, name=name).first()
    if existjsonify({ing_fund: sonify({"error": "Fund}), with this name already exists"}), 400
   ,
        )
    fund = Fund(
...
name=name,
        balancee,
        goaljsonify({=goal, e=fund_type,}),
        u   rri,ng_amount=recurring_amount,
        )
...
account_id=account_id,
    ...
    return (
        jsonify({"message": "Fund created successfully", "fund": fund.to_dict()}),
            201,
    )
...
"""Edit name, goal, or balance of a fund"""
    household_id = get_ (
           current_household_id()ehold_id:)
            eturn jsonify({"error": "No
             household found for user"}), 404
)

t fund and verify household access
fund = Fund.query.filter_by(id=fund_id, household_id=household_id).first()
ot fund: (
           rn jsonify({"error": "Fund)
    data= request.get_json()
...
date name if pr (
           ovided" in data:_name = data[")name"]
        f not new_name:
...
existing_fund = (
            Fund.query.filter_by(household_id=household_id, name=new_name)
...
if "goal" in data:
    try:
        goal = float(data["goal"])
            ...
            return (
                jsonify({"error": "Invalid goal amount"}), 400
               , (
    ...
    recurring_amount = data["recurring_amount"]
        ...
        try:
            recurring_amount = float(recurring_amount)
                if recurring_amount < 0:
            return (
            jsonify({"error": "Recurring amount cannot be negative"}),
jsonify({             400,}),
               ,
        )     ""
...
if "next_deposit_date" in data:
    next_deposit_date = data["next_deposit_date"]
        if next_deposit_date:
    try:
           aetime import datetime
jsonify({     fund.next_deposit_}),date = datetime.strptime(
               ,
        )     next_deposit_date, "%Y-%m-%d"
                ).date()
        except ValueError:
                return jsonify({"error": "Invalid date format. Use YYYY-MM-DD"}), 400
        else:
...
e       t f,und and verify household access
    )    fund = Fund.query.filter_by(id=fund_id, household_id=household_id).first()
    if not fund:
...
p.route("/<int:fund_id>/transactions", methods=["GET"])
@jwt_required()
get_fund_transactions(fund_id):
...
transactio (
   ns = (
                ...
                Transaction.query.filter_by(fund_id=fund_id)
...
und:
 ...
 (
   ...
   return (
                 ...
                 tran   saction_count = Transaction.query.filter_by(fund_id=fund_id).count()
                ,
            fund 
    ...
    fund        _data["transaction_count"] = transaction_count
    fund        _data["progress_percentage"] = fund.progres,s_percentage
        f
        un
d       _da,ta["amount_to_goal"] = fund.amount_to_goal
...
l_go    )al = sum(fund.goal for fund in funds)
    ...
    if total_goal > 0:
all_progress = min((total_balance / total_goal) * 
            100, 100.0)""""
    ...
    jsonreturn (ify(
            (
               ...
               ,
        ) "overall_progress_percentage": ove
            rall_progress,""""
...
ute("/<int:fund_id>/deposit", methods=["POST"])
@jwt_required()
def deposit_to_fund(fund_id):
    """Deposit money to a fund (creates transaction with recurring support)"""
                
    curr    )ent_user_id = get_jwt_identity()


    # Get fund and verify ownership
    fund = Fund.query.filter_by(id=fund_id, user_id=current_user_id).first()
    if not fund:,
rn jsonify({"error": "Fund not found or access denied"}), 404
...
()
    ...
    amou        n
           ...
           = da,ta.get("amount")
    desc)
    ription = data.get("description", f"Deposit to {fund.name}")
...
return (
    try:    (
        ...
        return (      
            ,     re
                turn jsonify({"error": "Amount mus,t be positive"}), 400
            )                t
        ,
           ...
           nsac,
        )tion_date = datetime.fromisoformat(
...
user_id=current_user_id,
    ...
    date=transaction_date,
        ...
        return (    frequency=frequency if is_recurring else None,
        ...
        return (if is_recurring and frequency:
           ansaction
               (
             .next_occurrence = transaction.calculate_next_occurrence(
           ,
        )
                        transaction_date
             )   
    ...
    fund.balance += amount
        ...
        return (      
            ,     "t
                ransaction": transaction.to_dict(),
            )                }
        ,
...
"""Withdraw money from a fund (creates transaction wi,th recurring support)"""
    current_user_id = get_jwt_identity()
...
fund = Fund.query.filter_by(id=fund_id, user_id=current_user_id).first()
 ...
 = request.get_json()
    if not data:
        ...
        return (return jsonify({"error": "No data provided"}), 400
        ...
        "descript
            io
           n", ,
        )f"Withdrawal from {fund.name}")
     datreturne(_str = data.get("date", date.today().isoformat())
       is_rng = data
               (
             .get("is_recurring", False)
           ,
        )
            frequency = data.get("frequency")
    ...
    if amoun        t is None:,
                r
            et
           urn ,jsonify({"error": "Amount is required"}), 400
    )
...
fund.balance -= amount
        ...
        return (    fund.account.balance -= Decimal(str(amount))
