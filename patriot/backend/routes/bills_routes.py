...
def get_bills():
    ...
    return (
        jsonify({"bills": [bill.to_dict() for bill in bills], "total": len(bills)}),
        200,
    )
...
def get_bill_categories():
    ...
    try:
        ...
        return jsonify({"categories": category_list, "total": len(category_list)}), 200
...
def get_schedule():
    ...
    try:
        ...
        return jsonify({"schedule": None, "startingBalance": None}), 200
